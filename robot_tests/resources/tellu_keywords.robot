...
Resource          ..${/}resources${/}tellu_variables.robot

*** Keywords ***
TellU Create New Contact Person
    [Arguments]    ${customer_business_id}=${DEFAULT_TEST_ACCOUNT_BUSINESS_ID}    ${address_type}=Physical    ${streetname}=${DEFAULT_STREET_NAME}    ${postal}=${DEFAULT_POSTAL_CODE}    ${city}=${DEFAULT_CITY}    ${country}=${DEFAULT_COUNTRY}
