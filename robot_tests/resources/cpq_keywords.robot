...
Add Nth Product To Cart (CPQ)
    ...
    ${target_product}=    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Execute Javascript    return document.evaluate("${xpath}[${i}]//p[contains(@class,'product-name')]",document, null, XPathResult.ANY_TYPE, null).iterateNext().innerText;
...
Click Create Assets (CPQ)
    ...
    Wait Until Keyword Succeeds    20 s    3 s    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Click Element    ${CPQ_CREATE_ASSETS}
    Wait Until Keyword Succeeds    20 s    3 s    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Page Should Not Contain Element    ${CPQ_CREATE_ASSETS}
...
Click Create Order (CPQ)
    ...
    Wait Until Keyword Succeeds    20 s    3 s    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Click Element    ${CPQ_CREATE_ORDER}
...
Click Create Quote (CPQ)
    ...
    Wait Until Keyword Succeeds    20 s    3 s    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Click Element    ${CPQ_CREATE_QUOTE}
...
Click Save Order (CPQ)
    ...
    Wait Until Keyword Succeeds    20 s    3 s    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Click Element    ${SAVE_ORDER_BUTTON}
...
Click View Record (CPQ)
    ...
    Wait Until Keyword Succeeds    20 s    3 s    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Click Element    ${VIEW_RECORD_BUTTON}
...
Close Missing Information Popup (CPQ)
    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Keyword Succeeds    30s    5s    Click Element    ${CLOSE_BUTTON}
    ...
    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Element Is Not Visible    //div[@class='slds-modal__container']    120s
...
Fill Required Information For Telia Koneliittymä Plus
    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Keyword Succeeds    20s    1s    Select From List By Value    ${ATTRIBUTE_EDIT_WINDOW}${REQUIRED_ATTRIBUTE}//select
    ...    1

Fill Required Information For Telia Cid
    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Keyword Succeeds    10s    1s    Input Text    ${ATTRIBUTE_EDIT_WINDOW}${REQUIRED_ATTRIBUTE}//input
    ...    1
...
Fill Required Information For Telia Yritysinternet
    ...
    #Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Keyword Succeeds
    #...    20s    1s    Select From List By Value    ${xpath}//select    1
    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Keyword Succeeds    20s    1s    Select From List By Value    ${ATTRIBUTE_EDIT_WINDOW}${REQUIRED_ATTRIBUTE}//div[./label[text()[contains(.,'Liittymän nopeus')]]]//select
    ...    1
    #Sopimusaika
    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Keyword Succeeds    20s    1s    Select From List By Value    ${ATTRIBUTE_EDIT_WINDOW}${REQUIRED_ATTRIBUTE}//div[./label[text()[contains(.,'Sopimusaika')]]]//select
    ...    1
...
Fill Required Information For Telia Yritysinternet Plus
    ...
    ...    1
    #Palvelutaso
    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Keyword Succeeds    20s    1s    Select From List By Value    ${ATTRIBUTE_EDIT_WINDOW}${REQUIRED_ATTRIBUTE}//div[./label[text()[contains(.,'Palvelutaso')]]]//select
    ...    1

Fill Required Information For Microsoft Office 365
    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Keyword Succeeds    20s    1s    Input Text    //label[text()[contains(.,'Lisenssien määrä')]]/following-sibling::div//input
    ...    1
    #press tab
...
Select Exact Product
    ...
    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Page Contains Element    //ng-include/div/div[1]//p[contains(text(), '${target_product}')]    20 s
    ${length}=    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Execute Javascript    return document.evaluate("count(//div[@layout-name='cpq-product-list']//p)", document, null, XPathResult.ANY_TYPE, null).numberValue;
    : FOR    ${i}    IN RANGE    ${length}
    \    ${exact_product}=    Run Keyword And Return Status    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Element Text Should Be    //div[@layout-name='cpq-product-list']//div[@index='${i}']//p[normalize-space()='${target_product}']
    \    ...    ${target_product}
    \    Exit For Loop If    ${exact_product}
    Run Keyword If    ${exact_product}    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Keyword Succeeds    20s    1s
    ...    Click Element    //div[@layout-name='cpq-product-list']//div[@index='${i}']//p[normalize-space()='${target_product}']/../../following-sibling::div//button[contains(text(),'Add to Cart')]
...
Select Sales Type For Order (CPQ)
    ...
    ${length}=    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Execute Javascript    return document.evaluate("count(//tr//select[contains(@ng-model,'p.SalesType')])", document, null, XPathResult.ANY_TYPE, null).numberValue;
    ...
    : FOR    ${i}    IN RANGE    ${length}
    \    Wait Until Keyword Succeeds    10s    1s    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Select From List By Label
    \    ...    //tr[${i+2}]/td[8]//select[contains(@ng-model,'p.SalesType')]    New Money-New Services
...
Verify That Product In Cart Is Correct
    ...
    ${product_in_cart}=    Set Variable    //div[contains(@class, 'cpq-cart-item-title')]//div[contains(text(), '${target_product}')]
...
Wait Until Additional Attributes Are Updated (CPQ)
    ...
    Sleep    0.5
    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Element Is Not Visible    //div[contains(@class,'slds-modal')]//div[@class='modal-content-position modal-spinner-position']    20s
...
Wait Until Filled Information Is Recognized (CPQ)
    ...
    Sleep    0.5
    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Element Is Not Visible    //div[contains(@class,'slds-modal')]//div[@class='modal-content-position modal-spinner-position']    20s
    Run Inside Iframe    ${OPPORTUNITY_FRAME}    Wait Until Element Is Not Visible    //div[@class='cpq-cart-item-root-product-messages']//span[contains(text(),'Required attribute missing')]    30s
