...
${MUBE_ADMIN_PASSWORD}    9XwHVkDt3FgT5511232449Rxb3
#9XwHVkDt3FgT5511232449Rxb2
...
MUBE Choose Button "New Contact Person" And Fill Mandatory Data
    [Arguments]    ${first_name}=${EMPTY}    ${last_name}=${EMPTY}    ${language}=${DEFAULT_LANGUAGE}    ${email}=${DEFAULT_EMAIL}    ${phone_number}=${DEFAULT_PHONE}
    ...
    MUBE Fill Mandatory Contact Person Values With First And Lastname    ${first_name}    ${TEST_CONTACT_PERSON_LAST_NAME}    ${language}    ${email}    ${phone_number}
...
MUBE Create New Contact Person For Business Customer
    ...
    MUBE Fill Address Information With Address Type    Physical    ${street_name}    ${street_number}    ${staircase}    ${apartment_door}    ${postal_code}
    ...    ${city}
...
MUBE Fill Address Information With Address Type
    [Arguments]    ${address_type}    ${street_name}=${DEFAULT_STREET_NAME}    ${street_number}=${DEFAULT_STREET_NUMBER}    ${staircase}=${DEFAULT_STAIRCASE}    ${apartment_door}=${DEFAULT_APPARTMENT}    ${postal_code}=${DEFAULT_POSTAL_CODE}
    ...    ${city}=${DEFAULT_CITY}
    ...
    Run Keyword If    '${address_type}' == 'Physical'    MUBE Set Address Information    ${street_name}    ${street_number}    ${staircase}    ${apartment_door}
...
MUBE Fill In Address Information
    [Arguments]    ${address_type}=Official Address    ${postal_code}=${DEFAULT_POSTAL_CODE}    ${city}=${DEFAULT_CITY}    ${street}=${DEFAULT_STREET_NAME}    ${street_number}=${DEFAULT_STREET_NUMBER}
...
MUBE Fill Mandatory Contact Person Values With First And Lastname
    [Arguments]    ${firstname}    ${lastname}    ${language}=${DEFAULT_LANGUAGE}    ${email}=${DEFAULT_EMAIL}    ${phone_number}=${DEFAULT_PHONE}
...
MUBE Filter History Of Contact Person By Attribute And Verify Change
    ...
    Run Keyword Unless    ${passed}    MUBE Set Filter Select    Attribute name    -- Any --
    MUBE Apply Second Filter In Page
    ${xpath}=    Set Variable If    '${attribute}'=='Main Phone Number'    //tr/td/span[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), translate('Phone Number', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'))]    //tr/td/span[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), translate('${attribute}', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'))]
...
MUBE Handle Contact Person With Email Exists Notification
    ...
    ${notification_exists}=    Run Keyword And Return Status    Page Should Contain    Possible duplicate Contact Persons found. Please select to create, if you want to create a new Contact Person
...
MUBE Open All Cases Page
    ...
    Wait Until Page Contains Element    ${DIV_CONTAINING_ALL_CASES_TEXT}    180s
...
MUBE Save Contact Person
    ...
    Run Keyword Unless    ${unvalidated_address} or ${update_button}    Run Keywords    Capture Page Screenshot
    ...    AND    Fail
...
MUBE Verify That Case Attributes Are Populated Correctly
    ...
    #17.9    MUBE Check History Old And New Value For Attribute    Product 1    ${EMPTY}    ${PRODUCT_TYPE}
    ...
    #17.9    MUBE Check History Old And New Value For Attribute    External system    ${EMPTY}    B2BSELFCARE
