...
def generate_docs(libdoc_args, libroot, resourceroot, docroot):
    ...
    for sourcefile in libs + resources:
        ...
        libdoc_args.extend([sourcefile, targetpath])
        try:
            libdoc_cli(libdoc_args)
...
def main(cli_args):
    ...
    if len(cli_args) > 1 and cli_args[0] == 'docs':
