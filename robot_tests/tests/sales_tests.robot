*** Settings ***
Suite Teardown    Close All Browsers    # Suite Setup    Open Browser And Go To Login Page
...
Sales Admin: Change Account owner
    ...
    Change Account Owner    Sales Admin
    Verify that Owner Has Changed    Sales Admin
    [Teardown]    Run Keywords    Revert Account Owner Back To GESB Integration
    ...    AND    Logout From All Systems and Close Browser
...
Contact: Update contact
    [Tags]    BQA-23
    [Setup]    Run Keywords    Open Browser And Go To Login Page
    ...    AND    Check If Contact Person Exists And Create New One If Not    ${CONTACT_PERSON_CRM_ID_FOR_UPDATE_TEST}
    ...    AND    Close Tabs And Logout
    Go To Salesforce and Login
...
Opportunity: Pick opportunity from queue
    [Tags]    BQA-37
    [Setup]    Run Keywords    Open Browser And Go To Login Page
    ...    AND    Go To Salesforce and Login    Customer Care User
    ...    AND    Go to Account    ${DEFAULT_TEST_ACCOUNT}
    ...    AND    Create New Opportunity For Customer
...
Enable Sales Person to rate Opportunity and Task Source Data Quality
    ...
    ...    AND    Go to Account    ${DEFAULT_TEST_ACCOUNT}
    ...    AND    Create New Opportunity For Customer
    ...    AND    Create New Task For Customer
    ...    AND    Assign Task To    B2B Digisales
    ...    AND    Logout From Salesforce
...
Wait Until Account Is In Salesforce And Go To Account
    Wait Until Keyword Succeeds    25 minutes    15 seconds    Run Keywords    Run Keyword And Ignore Error    Close All Tabs
    ...    AND    Go To Account    Test ${TEST_CONTACT_PERSON_LAST_NAME}
...
Wait Until Contact Person Is Found In MultiBella
    ...
    Wait Until Keyword Succeeds    15 minutes    15 seconds    MUBE Contact Person Should Be Found X Times    ${TEST_CONTACT_PERSON_LAST_NAME}    ${times}

Update Closed Opportunity Test Case
    [Arguments]    ${stage}    ${status}    ${original_stage}=Analyse Prospect
    Log to Console    ${status}
    ${passed}=    Set Variable    FAILED
    ${new_close_reason}=    Set Variable If    '${status}'=='Won'    01 Relationship    09 Customer Postponed
    ...
    [Teardown]    Run Keywords    Log to Console    ${passed}
    ...    AND    Logout From All Systems And Close Browser
...
Contact Person Should Be Updated In Salesforce
    ...
    Click Contact Person Details
    Verify That Contact Person Information is Correct    name=Test123 ${TEST_CONTACT_PERSON_LAST_NAME}    email=${TEST_CONTACT_PERSON_EMAIL}    language=${DEFAULT_LANGUAGE_UPDATED}    street_name=${EMPTY}    street_number=${EMPTY}    staircase=${EMPTY}
    ...    apartment_door=${DEFAULT_PO_BOX}    postal_code=${DEFAULT_POSTAL_CODE_UPDATED}    city=${DEFAULT_CITY_UPDATED}
