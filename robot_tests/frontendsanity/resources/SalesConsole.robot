...
*** Keywords ***
Go To Salesforce
    Go To    ${CONS_LOGIN_PAGE}
    Login Page Should Be Open

Login Page Should Be Open
    Wait Until Keyword Succeeds    10 seconds    1 second    Location Should Be    ${CONS_LOGIN_PAGE}
...
Login To Salesforce Lightning And Close All Tabs
    ...
    ${present}=  Run Keyword And Return Status    Element Should Be Visible   ${CONS_TABS_OPENED}
    Run Keyword If    ${present}    Close All Tabs
    Sleep    5s
    Reset to Home Tab

Login to Salesforce as DigiSales Lightning User
    Login To Salesforce Lightning    ${CONS_B2B_DIGISALES_LIGHT_USER}    ${CONS_PASSWORDCONSOLE}
...
Login to Salesforce Lightning
    ...
    Wait Until Page Contains Element    xpath=${CONS_LIGHTNING_ICON}    120 seconds
...
Switch to Lightning
    Sleep    15s
    Click Element    xpath=${CONS_CLASSIC_MENU}
    Page Should Contain Element    xpath=${CONS_SWITCH_TO_LIGHTNING}    30s
    Click Element    xpath=${CONS_SWITCH_TO_LIGHTNING}
...
Search Salesforce
    [Arguments]    ${item}
    Wait Until Page Contains element    xpath=${CONS_SEARCH_SALESFORCE}    20s
    Input Text    ${CONS_SEARCH_SALESFORCE}    ${item}
    Sleep    15s
    Press Enter On  ${CONS_SEARCH_SALESFORCE}
    Sleep    10s
    ${IsVisible}=   Run Keyword And Return Status    Element Should Be Visible   ${CONS_TABS_OPENED}//a[contains(@title, 'Search')]  20s
    run keyword unless  ${IsVisible}        Search Salesforce      ${item}
    Wait Until Page Contains element    xpath=${CONS_TABS_OPENED}//a[contains(@title, 'Search')]    30S
...
Account Should Be Open
    ...
    ${present}=  Run Keyword And Return Status    Element Should Be Visible   ${CONS_TABS_OPENED}//[contains(@class,'tabItem') and @title='${account_name}']
    Run Keyword If    ${present}    Click specific element      ${CONS_TABS_OPENED}//[contains(@class,'tabItem') and @title='${account_name}']
...
Click New Item For Account
    ...
    Wait Until Page Contains Element    ${CONS_NEW_ITEM_POPUP}    30s
...
Input Quick Action Value For Attribute
    ...
    Wait Until Element Is Visible    ${CONS_NEW_ITEM_POPUP}//label//*[contains(text(),'${field}')]    20s
    Input Text    xpath=${CONS_NEW_ITEM_POPUP}//label//*[contains(text(),'${field}')]//following::input    ${value}

Select Quick Action Value For Attribute
    [Arguments]     ${field}    ${value}
    Wait Until Element Is Visible    ${CONS_NEW_ITEM_POPUP}//span[contains(text(),'${field}')]//following::div//a[@class='select']   20s
    ...
    Select option from Dropdown with Force Click Element    ${CONS_NEW_ITEM_POPUP}//span[contains(text(),'${field}')]//following::div//a[@class='select']    //div[@class='select-options']//li//a[contains(text(),'${value}')]


Fill Mandatory Classification
    [Arguments]    ${account_name}=${CONS_LIGHTNING_TEST_ACCOUNT}
...
Click Save Button
    Click Element       ${CONS_SAVE_OPPORTUNITY}
...
Verify That Opportunity Is Saved And Data Is Correct
    ...
    ${account_name}=    Set Variable    //span[text()='Account Name']/../..//a[text()='${CONS_LIGHTNING_TEST_ACCOUNT}']
...
Close All Tabs
    ...
    @{locators}=     Get Webelements    xpath=${CONS_TABS_OPENED}//div[contains(@class,'close')]
    ${original}=       Create List
    :FOR   ${locator}   IN    @{locators}
    \       Run Keyword and Ignore Error    Close Tab

Close Tab
    ${visible}=     run keyword and return status   element should be visible  ${CONS_TABS_OPENED}//div[contains(@class,'close')]
    run keyword if  ${visible}
    ...     Click Element   xpath=${CONS_TABS_OPENED}//div[contains(@class,'close')]
...
Verify That Opportunity Is Found With Search And Go To Opportunity
    ...
    Wait until Page Contains Element    ${CONS_OPPORTUNITYNAME_TAB}//*[text()='${OPPORTUNITY_NAME}']     30s
    Verify That Opportunity Is Saved And Data Is Correct    ${CONS_OPPORTUNITYNAME_TAB}
...
Verify That Opportunity is Found From My All Open Opportunities
    ...
    Sleep       10s
    Wait Until Page Contains Element    ${CONS_OPPORTUNITYNAME_TAB}${oppo_name}      240s
...
Open Opportunities
    ...
    ${present}=  Run Keyword And Return Status    Element Should Be Visible   ${TCONS_ABS_OPENED}
    Run Keyword If    ${present}    Close All Tabs
    Select Correct Tab Type     ${CONS_OPPORTUNITIES}
    Wait Until Page Contains Element    ${CONS_OPPORTUNITIES_SECTION}    ${timeout}
    Sleep       10s
...
Reset to Home Tab
    ...
    Run Keyword If    ${present}    Close All Tabs
    Sleep       5s
    Select Correct Tab Type     ${CONS_HOME}
    Sleep       5s

Select Correct Tab Type
    [Arguments]         ${tab}
    Wait Until Element is Visible       ${CONS_SALES_CONSOLE_MENU}       20 seconds
    Sleep   5s
    force click element     ${CONS_SALES_CONSOLE_MENU}
    #Click Element     ${SALES_CONSOLE_MENU}
    ${CreateButtonVisible} =    Run Keyword And Return Status      Element Should Be Visible    ${tab}
    run keyword if      ${CreateButtonVisible}=='FAIL'    Click Element     ${CONS_SALES_CONSOLE_MENU}
    Sleep       3s
    Click Element               ${tab}
    Sleep       5s
...
Filter Opportunities By
    ...
    Wait Until Page Contains Element        ${CONS_SEARCH_INPUT}     20s
    Input Text          xpath=${CONS_SEARCH_INPUT}      ${value}
    Press Key       xpath=${CONS_SEARCH_INPUT}     \\13
    Sleep   5s
    Force click element     ${CONS_RESULTS_TABLE}[contains(@class,'forceOutputLookup') and (@title='${value}')]
...
Go to Contacts
    Go to Sales Console
    Wait Until Element is Visible       ${CONS_CONTACTS}                 20 seconds
    Click Element                       ${CONS_CONTACTS}
    ${present}=  Run Keyword And Return Status    Element Should Be Visible   ${CONS_TABS_OPENED}
    Run Keyword If    ${present}    Close All Tabs
...
Go to Sales Console
    Wait Until Element is Visible       ${CONS_SALES_CONSOLE_MENU}       20 seconds
    click element   ${CONS_SALES_CONSOLE_MENU}
...
Create New Master Contact and Validate
    Sleep       10s
    Click Element                           ${CONS_NEW_CONTACT}
    Click Visible Element                   //span[text()='Master']
    Click element                           //span[text()='Next']
    Wait Until Element is Visible           ${CONS_CONTACT_INFO}               20 seconds
    ...
    Input Text           ${CONS_MOBILE_NUM}         ${CONTACT_MOBILE}
    Input Text           ${CONS_FIRST_NAME_FIELD}         ${CONTACT_FIRST_NAME}
    Input Text           ${CONS_LAST_NAME_FIELD}          ${CONTACT_LAST_NAME}
    Select from Autopopulate List       ${CONS_ACCOUNT_NAME}         ${CONS_CONTACT_ACCOUNTNAME}
    sleep  10s
    Input Text              xpath=${CONS_PRIMARY_EMAIL}          ${CONTACT_PRIMARY_EMAIL}
    Click Element                           ${CONS_SAVE_BUTTON}
    Sleep       10s
...
Create New NP Contact and Validate
    ...
    Input Text                              ${CONS_MOBILE_NUM}             ${NP_CONTACT_MOBILE}
    Input Text                              ${CONS_FIRST_NAME_FIELD}       ${NP_CONTACT_FIRSTNAME}
    Input Text                              ${CONS_LAST_NAME_FIELD}        ${NP_CONTACT_LASTNAME}
    Select from Autopopulate List           ${CONS_ACCOUNT_NAME}           ${NP_CONTACT_ACCOUNTNAME}
    Input Text                              ${CONS_PRIMARY_EMAIL}          ${NP_CONTACT_EMAIL}
    sleep  10s
    Click Element                           ${CONS_SAVE_BUTTON}
    Sleep                                   10s
...
Create New Contact for Account and Validate
    click element           ${CONS_AP_NEW_CONTACT}
    sleep                   2s
    Input Text              ${CONS_AP_MOBILE_NUM}         ${AP_CONTACT_MOBILE}
    ${first_name}=  Run Keyword     Create Unique Name          ${EMPTY}
    ${email_id}=    Run Keyword     Create Unique Email         ${DEFAULT_EMAIL}
    ...
    Input Text              ${CONS_FIRST_NAME_FIELD}         ${AP_CONTACT_FIRSTNAME}
    Input Text              ${CONS_LAST_NAME_FIELD}          ${AP_CONTACT_LASTNAME}
    Input Text              ${CONS_PRIMARY_EMAIL}            ${AP_CONTACT_EMAIL}
    Click Element           ${CONS_AP_SAVE_BUTTON}
    Sleep                   2s
...
Switch to Lightning Sales Console
    wait until page contains element        ${CONS_APP_LAUNCHER}             15s
    click element                           ${CONS_APP_LAUNCHER}
    wait until page contains element  //div[@class="container"]//input[@type="search"]  60s
    input text  //div[@class="container"]//input[@type="search"]   Sales Console
    press enter on  //div[@class="container"]//input[@type="search"]
    sleep  30s
    ...
    wait until element is visible           ${CONS_SALES_APP_NAME}           15s
